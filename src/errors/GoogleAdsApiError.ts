...
export const extractGoogleAdsApiError = (error: Error) => {
  ...
  if (!isGrpcError(error)) throw new CouldNotExtractGoogleAdsApiError({ source: error });

  // try and grab the failure binary from the grcp error
  const googleAdsFailureBin = (error as any).meta?.['google.ads.googleads.v11.errors.googleadsfailure-bin'];
  if (!googleAdsFailureBin) throw new CouldNotExtractGoogleAdsApiError({ source: error });

  // decode the failure and cast to GoogleAdsApiError
  const failure = GoogleAdsFailure.fromBinary(Uint8Array.from(Buffer.from(googleAdsFailureBin, 'base64')));
